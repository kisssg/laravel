  ...
  <div class="modal-content" style="background-color: #cce8cf">
    ...
    <div class="modal-body">
      <table class="table table-bordered table-hover">
        <tbody>
          <tr
            v-for="(item, key, index) in data_to_score"
            :key="index"
            ...
            "
          >
            <span style="border: 1px solid #515151"
              ><span
                style="
          ...
          <tr v-for="(item, key, index) in items" :key="index">
            <td v-if="item.option_type === 'text'">
              <text-score-item
                :item="item"
                :savedAnswer="score ? score[item.score_field] : null"
                :ref="item.score_field"
              ></text-score-item>
            </td>
            <td v-else-if="item.option_type === 'single'">
              <single-select-score-item
                :item="item"
                :savedAnswer="score ? score[item.score_field] : null"
                :ref="item.score_field"
              ></single-select-score-item>
            ...
            <td v-else>
              Option type <b>{{ item.option_type }}</b> not supported, please
              verify.
            </td>
          </tr>
          <tr>
          ...
          <tr v-if="score">
            <td>
              <span>{{
                score.created_by + " created at " + score.created_at + ". "
              }}</span>
              <span class="float-right" v-if="score.updated_by">{{
                score.updated_by + " updated at " + score.updated_at
              }}</span>
            </td>
          </tr>
        </tbody>
...
export default {
  ...
  methods: {
    getItems: _.debounce(function (id, vm) {
      axios.get("item?json=1&project=" + id).then(
        (res) => {
          vm.items = res.data;
        },
        (error) => {
    ...
    submitResult() {
      ...
      for (let item in scoreData) {
        ...
        if (score[item] !== "-" && scoreData[item][0].needScore) {
          scoreDenominator++;
          if (score[item] === "1") scoreNumerator++;
        }
        if (scoreData[item][0].needScore)
          totalScore +=
            scoreData[item][0].answer === null
              ? 0
              : Number(scoreData[item][0].score);
      }
      ...
      totalScore =
        this.data_to_score.score_method === "1"
          ? totalScoreFactor
          : totalScoreFixed;
