<template>
  <div class="overflow-auto">
    <span style="font-family: Verdana; font-size: 18px; font-weight: normal">{{
      item.order + "." + item.title
    }}</span>
    <span v-if="deprecated_answer" class="text-danger"
      >({{ deprecated_answer }})</span
    ><br />
    <span v-if="show_description" style="font-family: Verdana;font-size: 9px;" class="text-muted">{{item.sub_title}}</span>
...
export default {
  ...
  computed: {
    score() {
      if (this.needScore === false) return null;
    ...
    show_description() {
      ...
      return urlParams.has("show_description"); //
    },
  },
};
</script>